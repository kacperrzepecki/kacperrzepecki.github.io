...
var parallaxPage = function(name, pageHtml){
	return{
		key: name,
		page : pageHtml,
		right  : function(callback){return this.transition({left:parallax.width,top:0}  ,{left:-2*parallax.width,top:0} ,callback);},
		left   : function(callback){return this.transition({left:-parallax.width,top:0} ,{left:2*parallax.width,top:0}  ,callback);},
		...
		transition : function(locationNew, locationOld, callback){
            ...
            thisPage.page.css("display", "block");
            ...
            var prevPage = parallax.current;
						parallax.current.slide( locationOld,
							function(){
								parallax.sliding = false;
                prevPage.page.css('display', 'none');
							}
						);
					}
	...
	add : function(key,object) {
		...
		if(check){
			...
			if(validKeyName){
				...
				this[key].hide();
				this[key].page.css("position","fixed");
			}else{
